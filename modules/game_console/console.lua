...
function loadLootList()
	-- moje zabawy Loot Filterem
	local count = 1
	local itemName = ""
	local textColor = ""
	local errCount = 0
	local tempSpeakType = SpeakTypesSettings.channelOrange.speakType
	itemsToLoot = {}

	c_dir = g_resources.getWorkDir() .. 'lootlist.txt'
	--print('directory: ' .. c_dir)
	myFile = io.open(c_dir, "r")
	...
	if myFile ~= nil then
		if myFile:seek('end') > 0 then
			for line in io.lines(c_dir) do
				if line:len() > 3 then
					if line:find('=') then
						itemName = string.sub(line, 1,line:find("=")-1):trim()
						textColor = string.sub(line, line:find("=")+1,line:len())
						if itemName:len() > 0 and textColor:len() > 1 then
							itemName = itemName:trim()
							if itemName:find('%%') ~= nil then
								itemName = string.sub(itemName, 0, itemName:find('%%') - 1)
							end
							print(itemName)
							textColor = textColor:trim()
							if string.find(textColor, '#') ~= nil then
								itemsToLoot[count] = {itemName = string.lower(itemName), textType = {tempSpeakType, color = textColor}}
								count = count + 1
							else
								for x = 1, #lootFilterColors do
									if string.lower(textColor) == string.lower(lootFilterColors[x].color) then
										itemsToLoot[count] = {itemName = string.lower(itemName), textType = {tempSpeakType, color = lootFilterColors[x].hexColor}}
										count = count + 1
										break
									end
								end
							...
							print('Problem w linijce:')
							print('==> ' .. line .. ' <==')
						end
					else
						errCount = errCount + 1
						print('Blad nr ' .. errCount .. ' w linijce: ' ..line)
					end
				else
					errCount = errCount + 1
					print('Blad nr ' .. errCount .. ':' .. 'Linia jest zbyt krotka.')
				end
			end
			if count == 1 then
				enableFilter = false
				print('Wystapil problem z ladowaniem pliku lootlist.txt.')
			end
		end
		myFile = io.close(myFile)
	else
		print('Stworz plik lootlist.txt w glownym folderze OTClienta!')
		enableFilter = false
	end
end

function loadFilterColors()
	-- moje zabawy Loot Filterem
	enableFilter = true
	local count = 1
	local itemName = ""
	...
	local c_dir = g_resources.getWorkDir() .. 'kolory.txt'
	--print('directory: ' .. c_dir)
	local myFile = io.open(c_dir, "r")
	
	if myFile ~= nil then
		if myFile:seek('end') > 0 then
			for line in io.lines(c_dir) do
				if line:len() > 3 and line:find('=') and line:find ('#') then
					local pos = line:find('=')
					local colorName = string.sub(line, 1, pos-1)
					local colorHex = string.sub(line, pos+1, line:len())
					if colorName:len() ~= nil and colorHex:len() ~= nil then
						local tempTable = {}
						colorName = colorName:trim()
						colorHex = colorHex:trim()
						if string.lower(colorName) == 'default' then
							defaultLootColor = colorHex
						end
						lootFilterColors[count] = {color = colorName, hexColor = colorHex}
						count = count + 1
					end
				end
			end
		...
		print('Problem z plikiem kolory.txt')
	end
...
function addText(text, speaktype, tabName, creatureName)
  ...
  if tab ~= nil then
	if string.lower(g_settings.get('host')) == 'armia.toproste.pl' then
		if (tabName == 'Loot-Channel')  and (enableFilter == true) then
			text = text:sub(text:find(":")+1, text:len())
			text = text:trim()
			if text:len() ~= nil then
				speaktype.color = defaultLootColor
				addLootChannelText(text, speaktype, tab, creatureName)
			end
		else
...
function addLootChannelText(text, speaktype, tab, creatureName)
	if not tab or tab.locked or not text or #text == 0 then return end
	local textStart = text:len()
	text = '[' ..creatureName .. '] - ' .. text
	if modules.client_options.getOption('showTimestampsInConsole') then
		text = os.date('[%H:%M] @ ') .. text
	end
	textStart = text:len() - textStart
	textLower = string.lower(text)
	
	--CREATE NEW LINE IN LOOT-CHANNEL TAB
	local panel = consoleTabBar:getTabPanel(tab)
	local consoleBuffer = panel:getChildById('consoleBuffer')
	local label = g_ui.createWidget('ConsoleLabel', consoleBuffer)
	label:setId('consoleLabel' .. consoleBuffer:getChildCount())
	label:setText(text)
	label:setColor(defaultLootColor)
	consoleTabBar:blinkTab(tab)
	
	--SEARCH FOR AN ITEMS IN TEXT SENT FROM THE SERVER
	for i = 1, #itemsToLoot do
		local itemPos = textStart
		local tempName = itemsToLoot[i].itemName
		local tempText = ''
		local tempItem = ''
		local hPos = 1
		local itemC = 1
		local itemStart = textStart
		
		while textLower:find(tempName, itemPos+1) do
			local endPos = 0
			local tempChar = ''
			while true do
				endPos = textLower:find(',', itemStart + 1)
				if not endPos then
					endPos = text:len()+1
				end
				if textLower:sub(itemStart, endPos):find(tempName) then
					tempItem = text:sub(itemStart, endPos-1)
					itemPos = itemStart
					itemStart = endPos
					break
				end
			...
			--FILL COLOURED  LABEL WITH EMPTY SPACES
			for letter = hPos, itemPos-1 do
				tempChar = string.byte(text:sub(letter,letter))
				local fillChar =  (tempChar == 10 or tempChar == 32) and string.char(tempChar) or string.char(127)
				tempText = tempText .. string.rep(fillChar, letterWidth[tempChar])
			end
			tempText = tempText .. tempItem
			hPos = itemStart
			itemPos = itemStart + 1
		end
		--CREATE COLOURED LABEL
		...
		labelHighlight:setColor(itemsToLoot[i].textType.color)
		labelHighlight:setText(tempText)
	end
...
function processChannelTabMenu(tab, mousePos, mouseButton)
  ...
  if tab ~= defaultTab and tab ~= serverTab then
	if channelName == 'Loot-Channel' then
		menu:addOption(tr('Zaladuj Filter'), function() 
		loadFilterColors()
		loadLootList() 
		end)
		if enableFilter == true then
			menu:addOption(tr('Wylacz Filter'), function()
			loadFilterColors()
			loadLootList()
			enableFilter = false
			end)
		else
			menu:addOption(tr('Wlacz Filter'), function()
			enableFilter = true
			end)
		end
	end
  ...
  elseif speaktype.private then
    ...
    if channel then
      addText(composedMessage, speaktype, channel, name)
		...
		if (channelId == 9) and (enableFilter == true) then
			onOpenChannel(channelId, 'Loot-Channel')
		else
